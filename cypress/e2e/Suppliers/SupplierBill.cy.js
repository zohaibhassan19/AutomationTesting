...
it('Create Bill with Purchase order',()=>{
    ...
    cy.contains('.MuiListItemButton-root', "Bills") .trigger('mouseover') .click({ force: true });
    ...
    // Fill form 
     ...
     cy.get('[class*="-option"]').eq(1).click({ force: true })
     cy.get('[name="referenceNumber"]') .clear() .type(randomRef)

     // Parts Fields
     cy.get(':nth-child(1) > :nth-child(1) > .MuiGrid-root > .gap-2 > .MuiButtonBase-root').click({ multiple: true, force: true })

     cy.get('[name="purchaseInvoiceItem.0.itemName"]') .closest('.commonSelect') .find('input') .first() .click({ force: true })
     cy.get('[class*="-option"]').eq(0).click({ force: true })

    cy.get('[name="purchaseInvoiceItem.0.quantity"]') .type(5)
     cy.get('[name="purchaseInvoiceItem.0.unitPrice"]').type(100)
     cy.get('[name="purchaseInvoiceItem.0.discount"]').type(5)
