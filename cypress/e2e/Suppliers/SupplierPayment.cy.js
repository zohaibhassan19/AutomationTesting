...
it('Supplier Payment',()=>{
    ...
    cy.contains('.MuiListItemButton-root', "Supplier Payments") .trigger('mouseover') .click({ force: true });
    ...
    cy.contains('Suppliers *').closest('.commonSelect').find('input').first().click({ force: true })
     ...
     cy.contains('Adjustment Type *').closest('.commonSelect').find('input').first().click({ force: true })
     cy.get('[class*="-option"]').eq(0).click({ force: true })
